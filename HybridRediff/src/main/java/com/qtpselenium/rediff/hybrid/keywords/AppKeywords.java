...
public class AppKeywords extends GenericKeywords{
	...
	public void validateLogin(){
		...
		boolean result = isElementPresent(objectKey);
		if(result)
		{
			takeSceenShotForPass();
		}else {
			takeSceenShotForFail();
		}
	...
	public void verifyAccountCreation()
	{
		...
		if(result)
		{
			...
			reportFailure("Account is not reated");
			takeSceenShotForFail();
		}
	}
	public void selectJourneyDate() throws ParseException
	{
		String monthToSelect=null;
		String yearToSelect=null;
		Date convertedFormat=new SimpleDateFormat("dd/MM/yyyy").parse(dataKey);
		String givenDate=new SimpleDateFormat("dd").format(convertedFormat);
		String givenMonth=new SimpleDateFormat("MMMM").format(convertedFormat);
		String givenYear=new SimpleDateFormat("yyyy").format(convertedFormat);
		//System.out.println(convertedFormat);
		while(true)
		{
		//List<WebElement> allDateRows=null;
		//List<WebElement> allDatevalues=null;

			monthToSelect=driver.findElement(By.xpath("//*[@id=\"modifcal\"]/table/thead[2]/tr[1]/td[2]")).getText().split(",")[0];
			yearToSelect=driver.findElement(By.xpath("//*[@id=\"modifcal\"]/table/thead[2]/tr[1]/td[2]")).getText().split(",")[1];
		//String []actualDate=text.split(",");
		//System.out.println(text);
		//i=2;
				if(yearToSelect.trim().equals(givenYear.trim()) && monthToSelect.trim().equals(givenMonth.trim()))
					{
						driver.findElement(By.xpath("//td[text()='"+givenDate+"']")).click();
						break;
					}else {
						driver.findElement(By.xpath("//*[@id=\"modifcal\"]/table/thead[2]/tr[1]/td[6]")).click();
					}
		
		}
	}
	public void enterStartPlace() throws InterruptedException
	{
		type(objectKey,dataKey);
		Thread.sleep(2000);
		driver.findElement(By.xpath("//*[@id=\"ui-id-1\"]/li/a[text()='"+dataKey+"']")).click();
		//pressEnterKey();
	}
	public void enterEndPlace() throws InterruptedException
	{
		type(objectKey,dataKey);
		Thread.sleep(2000);
		driver.findElement(By.xpath("//*[@id=\"ui-id-2\"]/li/a[text()='"+dataKey+"']")).click();
		//pressEnterKey();
	}
	public void verifyBusSearched()
	{
